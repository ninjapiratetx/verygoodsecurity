...
from src.static_values import BODY, ID, TEST_USERID_NOT_IN_POSTS, TITLE, TEST_BODY, TEST_TITLE, TEST_USERID, USERID
...
def test_post_userId_not_in_list():
    last_post_json_before = get_last_post()
    post_json = post_the_post(TEST_TITLE, TEST_BODY,TEST_USERID_NOT_IN_POSTS)
    assert post_json[TITLE] == TEST_TITLE
    assert post_json[BODY] == TEST_BODY 
    assert post_json[USERID] == TEST_USERID_NOT_IN_POSTS
    assert post_json[ID] == last_post_json_before[ID]+1
